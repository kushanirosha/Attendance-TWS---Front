    ...
    <div className="bg-white rounded-xl shadow-md border border-gray-100">
      ...
      <div className="p-6 border-b border-gray-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex space-x-2">
          {/* Save Button - Hidden/Disabled for STL */}
          {!isReadOnly && (
            <motion.button
                ...
                >
                  ...
                  {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((day) => {
                    ...
                    return (
                      ...
                      >
                        {isReadOnly ? (
                          // Read-only mode: just show the shift text
                          <div
                            className={`w-full px-1 py-1.5 text-sm font-bold text-center rounded ${getShiftColor(
                              currentValue
                            )}`}
                          >
                            {currentValue || "-"}
                          ...
                          // Editable mode: select dropdown
                          <select
                            value={currentValue}
                            onChange={(e) =>
                              handleCellChange(employee.id, dayStr, e.target.value)
                            }
                            className={`w-full px-1 py-1.5 text-sm font-semibold text-center border-0 rounded focus:ring-2 focus:ring-blue-500 cursor-pointer ${getShiftColor(
                              currentValue
                            )}`}
                          >
                            <option value="">-</option>
